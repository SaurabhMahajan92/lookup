...
import { updateFavoriteDistributionList, deleteFavoriteDistributionList } from '../../apis/apiList';
import Pagination from '../Pagination/Pagination';
import { AxiosResponse } from "axios";
import './DistributionLists.scss';
import { IUserPageSizeChoice } from "./../DistributionListMembers/DistributionListMembers"
...
export interface IDistributionListsState {
    ...
    countSortIcon: IconDefinition,
    onlineCountSortIcon: IconDefinition,
    loader: boolean;
    pageSize: number;
}

export default class DistributionLists extends React.Component<IDistributionListsProps, IDistributionListsState> {
    ...
    constructor(props: IDistributionListsProps) {
        ...
        this.state = {
            ...
            nameSortIcon: faSortAmountDown, //Sort flag images
            aliasSortIcon: faSortAmountDown,
            countSortIcon: faSortAmountDown,
            onlineCountSortIcon: faSortAmountDown,
            ...
            this.props.getFavoriteDistributionLists().then((response: AxiosResponse<IDistributionList[]>) => {
                ...
                for (let i = 0; i < favorites.length; i++) {
                    distributionLists.push({
                        ...
                        noOfContacts: favorites[i].noOfContacts,
                        noOfContactsOnline: this.notYetFetchedText,
    ...
    private getAllDistributionListMembersOnlineCount = async () => {
        this.state.masterDistributionLists.forEach((currentDistributionList) => {
            if (currentDistributionList.noOfContactsOnline === this.notYetFetchedText) {
            ...
            this.setState(state => {
                const masterDistributionLists: IDistributionList[] = state.masterDistributionLists.map((currentItem) => {
                    if (distributionList.id === currentItem.id) {
                        currentItem.noOfContactsOnline = response.data;
                    }
                    return currentItem;
                });
                return {
    ...
    private changePinStatus = (e: React.MouseEvent<HTMLAnchorElement, MouseEvent>) => {
        ...
        const distributionList = (this.state.distributionLists.filter((x: IDistributionList) => { return x.id === distributionListId }));
    ...
    private sortDataByColumn = (column: string, currentIcon: IconDefinition) => {
        if (currentIcon === faSortAmountDown) {
            switch (column) {
                case "displayName":
                    this.sortColumnItems(faSortAmountUp, faSortAmountDown, faSortAmountDown, faSortAmountDown, "displayName", false);
                    break;
                case "mail":
                    this.sortColumnItems(faSortAmountDown, faSortAmountUp, faSortAmountDown, faSortAmountDown, "mail", false);
                    break;
                case "noOfContacts":
                    this.sortColumnItems(faSortAmountDown, faSortAmountDown, faSortAmountUp, faSortAmountDown, "noOfContacts", false);
                    break;
                case "noOfContactsOnline":
                    this.sortColumnItems(faSortAmountDown, faSortAmountDown, faSortAmountDown, faSortAmountUp, "noOfContactsOnline", false);
                    break;
            }
        }
        else {
            this.sortColumnItems(faSortAmountDown, faSortAmountDown, faSortAmountDown, faSortAmountDown, column, true);
        }
    }

    //Setting the sort icons and sorting pinned-unpinned records separately
    private sortColumnItems = (faName: IconDefinition, faAlias: IconDefinition, faContactsCount: IconDefinition, faOnlineCount: IconDefinition, sortColumn: string, sortOrder: boolean) => {
        ...
        this.setState({
            ...
            countSortIcon: faContactsCount,
            onlineCountSortIcon: faOnlineCount,
            distributionLists: distributionLists,
        })
    ...
    private resetSorting = (favoriteDLs: IDistributionList[]) => {
        ...
        let sortColumn = "displayName"; //Default sort Column
        let isAsc = true;//Default sort order
        ...
        else if (this.state.countSortIcon === faSortAmountUp) {
            sortColumn = "noOfContacts";
            isAsc = false;
        }
        else if (this.state.onlineCountSortIcon === faSortAmountUp) {
            sortColumn = "noOfContactsOnline";
            isAsc = false;
        }

        pinnedRecords = orderBy(pinnedRecords, [sortColumn], isAsc === true ? ["asc"] : ["desc"]);
        unpinnedRecords = orderBy(unpinnedRecords, [sortColumn], isAsc === true ? ["asc"] : ["desc"]);
    ...
    private searchFavoriteDLs = (e: React.SyntheticEvent<HTMLElement, Event>) => {
        ...
        if (!searchQuery) // If Search text cleared
        {
            ...
            this.setState({
                distributionLists: this.state.masterDistributionLists.filter((x: IDistributionList) => x.displayName.toLowerCase().includes(searchQuery.toLowerCase())),
    ...
    public onOpenTaskModule = () => {
        if (this.isOpenTaskModuleAllowed) {
            ...
            const taskInfo: ITaskInfo = {
                url: url,
                title: "Add Favorite Distribution List",
                height: 650,
                width: 700,
                fallbackUrl: url,
            }
    ...
    //Open HelpPage with deep link
    private openHelpPage = () => {
        const encodedWebUrl = getBaseUrl() + "/help";
        this.props.getClientId().then((result: AxiosResponse<string>) => {
            const clientId = result.data;
            const taskItemUrl = 'https://teams.microsoft.com/l/entity/' + clientId + '/help?webUrl=' + encodedWebUrl;
            microsoftTeams.executeDeepLink(encodeURI(taskItemUrl));
        });
    }
    ...
    public render(): JSX.Element {
        ...
        for (let j: number = this.state.activePage * this.state.pageSize; j < (this.state.activePage * this.state.pageSize) + this.state.pageSize; j++)  // 20 is records per page
        {
            ...
            items.push(<Segment >
                <Flex gap="gap.small">
                    <FlexItem grow>
                        <Text content={this.state.distributionLists[j].displayName} />
                    </FlexItem>
                    <FlexItem>
                        <Icon iconName="Pinned" hidden={!this.state.distributionLists[j].isPinned} />
                    </FlexItem>
                </Flex>
            </Segment>)
            items.push(<Segment content={this.state.distributionLists[j].mail} ></Segment>)
            items.push(<Segment content={this.state.distributionLists[j].noOfContacts} ></Segment>)

            if (this.state.distributionLists[j].noOfContactsOnline === this.notYetFetchedText)
                items.push(<Segment><Loader size="smallest" /> </Segment>)
            else
                items.push(<Segment ><Text content={this.state.distributionLists[j].noOfContactsOnline} /></Segment>)

            items.push(<Segment >
                <Flex gap="gap.small">
                    <Anchor href={"/dlmemberlist/" + this.state.distributionLists[j].id + "/" + this.state.distributionLists[j].displayName}> View</Anchor>&nbsp;|&nbsp;
                    <Anchor className="seperatorSpacing" id={this.state.distributionLists[j].id} href="#" onClick={this.changePinStatus}> {this.state.distributionLists[j].isPinned ? "Unpin" : "Pin"}</Anchor>&nbsp;|&nbsp;
                    <Anchor className="seperatorSpacing" href="#" id={this.state.distributionLists[j].id} onClick={this.deleteFavorites}> Hide</Anchor>
                </Flex>
            </Segment>)
        }
        ...
        if (!this.state.loader && this.state.distributionLists.length === 0 && this.state.masterDistributionLists.length === 0)// If there are no favorites saved
        {
            return (<div className={"emptydiv"}>
                <Text content="Get started by adding an existing distribution list now." />
                <br />
                <br />
                <Button content="Add Distribution List" onClick={() => this.onOpenTaskModule()} primary />
                <br />
                <br />
                <Text content="Not ready yet, try the" /> <Anchor href="#" onClick={this.openHelpPage}> Tour</Anchor>
            </div>);
        }
        else {
            return (
                <div className="mainComponent">
                    <div className={"formContainer"}>
                        <Flex space="between">
                            <FlexItem grow>
                                <Text content="Distribution Lists" size={"larger"} weight="semibold" className="textstyle" />
                            </FlexItem >
                            ...
                            <Flex gap="gap.small">
                                <div className="divstyle">
                                    <Dropdown
                                        ...
                                        checkable
                                    /></div>
                                <Button aria-label="Add Distribution List" content="Add Distribution List" onClick={() => this.onOpenTaskModule()} primary />
                                <Input aria-label="Search" icon="search" placeholder="Search..." onChange={this.searchFavoriteDLs} className="textstyle" />
                            </Flex>
                        </Flex>
                        <br />
                        <div className="formContentContainer" >

                            <Grid columns="2fr 2fr 1.3fr 1.3fr 1.5fr">
                                <Segment color="brand" >
                                    <Flex gap="gap.small">
                                        <FlexItem grow>
                                            <Text content="Name" />
                                ...
                                <Segment color="brand" >
                                    <Flex gap="gap.small">
                                        ...
                                        <Anchor href="#" onClick={() => this.sortDataByColumn("noOfContacts", this.state.countSortIcon)}>
                                            <FlexItem push >
                                                <FontAwesomeIcon icon={this.state.countSortIcon} />
                                            </FlexItem >
                                        </Anchor>
                                    </Flex>
                                </Segment>
                                ...
                                <Segment color="brand" >
                                    <Flex gap="gap.small">
                                        ...
                                        <FlexItem grow>
                                            <Text content="Members Online" />
                                        </FlexItem>
                                        <Anchor href="#" onClick={() => this.sortDataByColumn("noOfContactsOnline", this.state.onlineCountSortIcon)}>
                                            <FlexItem push >
                                                <FontAwesomeIcon icon={this.state.onlineCountSortIcon} />
                                            </FlexItem >
                                        </Anchor>
                                    </Flex>
                                </Segment>
